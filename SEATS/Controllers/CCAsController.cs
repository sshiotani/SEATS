    ...
    public class CCAsController : Controller
    {
        ...
        private async Task EmailAdmin(CCA cca)
        {
            try
            {
                ...
                var initial = cca.Student.StudentFirstName[0];
                await userManager.SendEmailAsync(admin.Id, "<html>Admin Notification:New SEOP Application", "EDONLINE, A new application for SOEP has been received from <p>" + initial + ". " + cca.Student.StudentLastName + " Email:" + cca.Student.StudentEmail + "</p><p>This email was also sent to:</p><p>Parent:" + cca.Student.Parent.GuardianEmail + "</p></html>");
            }
            catch
            {
                throw;
